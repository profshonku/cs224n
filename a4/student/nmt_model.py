...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        ###     LSTM:nn.Linear(2*hidden_size, hidden_size, bias=False)
        ...
        self.post_embed_cnn = nn.Conv1d(in_channels=embed_size,out_channels=embed_size, kernel_size=2,padding="same",device=self.device)
        self.encoder = nn.LSTM(input_size = embed_size, hidden_size = hidden_size, bias = True, bidirectional = True,device=self.device) # default bias = True
        self.decoder = nn.LSTMCell(input_size = embed_size+hidden_size, hidden_size = hidden_size, bias = True, device=self.device)
        self.h_projection = nn.Linear(2*hidden_size, hidden_size, bias=False,device=self.device)
        self.c_projection = nn.Linear(2*hidden_size, hidden_size, bias=False,device=self.device)
        self.att_projection = nn.Linear(2*hidden_size, hidden_size, bias=False,device=self.device)
        self.combined_output_projection = nn.Linear(3*hidden_size, hidden_size, bias=False,device=self.device)
        self.target_vocab_projection = nn.Linear(hidden_size, len(vocab.tgt), bias=False,device=self.device)
